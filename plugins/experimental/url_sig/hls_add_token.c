...
char *rewrite_line(char *schema, char *host,
                   char *usePathNoFilename, char *line,
                   char *hashQuery, char *query,
                   char *key, int algorithm)
{
    char *res = NULL;
    ...
    {
        // Process #EXT-X-MAP.
        if (strncmp(line, "#EXT-X-MAP", 10) == 0 || strncmp(line, "#EXT-X-PART", 11) == 0 || strncmp(line, "#EXT-X-PRELOAD-HINT", 19) == 0 || strncmp(line, "#EXT-X-RENDITION-REPORT", 23) == 0)
        {
...
char *getSubstring(char *startPos, char *endPos)
{
...
char *processURI(char *schema, char *host,
                 char *usePathNoFilename, char *line,
                 char *hashQuery, char *query,
                 char *key, int algorithm)
{
    ...
    char *urlWithToken = createUrlWithSignHttpBase(
        schema, host, usePathNoFilename, getSubstring(startPos, endPos), hashQuery, query,
        key, algorithm);
...
char *createUrlWithSignHttpBase(char *schema, char *host,
                                char *usePathNoFilename, char *filename,
                                char *hashQuery, char *query,
                                char *key, int algorithm)
{
...
char *generateUrlWithoutQuery(char *schema, char *host,
                              char *usePathNoFilename, char *filename)
{
    str_builder_t *sb;
    sb = str_builder_create();
