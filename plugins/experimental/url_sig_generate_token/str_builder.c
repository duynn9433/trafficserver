...
void
str_builder_destroy(str_builder_t *sb)
{
  if (sb == NULL)
    return;
  free(sb->str);
 ...
 */
static void
str_builder_ensure_space(str_builder_t *sb, size_t add_len)
{
...
void
str_builder_add_str(str_builder_t *sb, const char *str, size_t len)
{
...
void
str_builder_add_char(str_builder_t *sb, char c)
{
  if (sb == NULL)
    return;
...
void
str_builder_add_int(str_builder_t *sb, int val)
{
  char str[12];
...
void
str_builder_clear(str_builder_t *sb)
{
  if (sb == NULL)
    return;
...
void
str_builder_truncate(str_builder_t *sb, size_t len)
{
...
void
str_builder_drop(str_builder_t *sb, size_t len)
{
  if (sb == NULL || len == 0)
    return;
...
size_t
str_builder_len(const str_builder_t *sb)
{
  if (sb == NULL)
    return 0;
  return sb->len;
...
const char *
str_builder_peek(const str_builder_t *sb)
{
  if (sb == NULL)
    return NULL;
  return sb->str;
...
char *
str_builder_dump(const str_builder_t *sb, size_t *len)
{
  char *out;
